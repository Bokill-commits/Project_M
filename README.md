...
Chrome DevTools를 활용하여 브라우저 트래픽을 수집하고, 행위 기반 Feature를 통해 의심 트래픽을 탐지합니다.
...
```
Project_M/
├── extension/                 # Chrome Extension 핵심 로직 및 UI 구성
│   ├── background.js
│   ├── devtools.html
│   ├── devtools.js
│   ├── features.js
...
│   └── model.json
├── tfjs/                      # 브라우저 머신러닝 모델 실행용
│   ├── tf-backend-cpu.min.js
│   ├── tf-core.min.js
│   └── tf-layers.min.js
└── generate_mock_data.ipynb   # 모의 데이터
```
